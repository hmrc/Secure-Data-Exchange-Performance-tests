...
import uk.gov.hmrc.perftests.requests.Requests.baseUrl
import utils.RequestUtils

object ADDERRequst extends ServicesConfiguration {
...
//  }
  def postPhysicaltransfer: HttpRequestBuilder = {
    http("PhysicalTransfer")
      .post(s"$baseUrl/sdes/create-request/how-move-data")
      .formParam("field", "OTHER")
      .formParam("csrfToken", "${csrfToken}")
      .check(status.is(303))
      .check(header("Location").is(s"/sdes/create-request/move-data-steps":String))
  }

 def getSteps: HttpRequestBuilder = {
    http("MovedataSteps")
     .get(s"$baseUrl/sdes/create-request/move-data-steps")
     .check(status.is(200))
     .check(RequestUtils.saveCsrfToken)
     .check(regex("What steps will you take to move the data").exists)
}
  def postStepstomoveData: HttpRequestBuilder = {
    http("StepstomoveData")
      .post(s"$baseUrl/sdes/create-request/move-data-steps")
  ...
  def postProtectedYes: HttpRequestBuilder = {
    http("ProtectedYes")
...
//  }
  ...
  def getStartdate: HttpRequestBuilder = {
    http("StartDate")
  ...
  def postStartDate: HttpRequestBuilder = {
    http("MovementStartDate")
...
//  }
  ...
  def getDatafrom: HttpRequestBuilder = {
    http("DataFrom")
      .get(s"$baseUrl/sdes/create-request/where-data-from")
      .check(status.is(200))
      .check(RequestUtils.saveCsrfToken)
      .check(regex("Where is the data coming from").exists)
  }
  def postDataFrom: HttpRequestBuilder = {
    http("EnterDataFrom")
      .post(s"$baseUrl/sdes/create-request/where-data-from")
      .formParam("field", "Test")
      .formParam("csrfToken", "${csrfToken}")
      .check(status.is(303))
      .check(header("Location").is(s"/sdes/create-request/where-data-to":String))
  }
  def getDataTo: HttpRequestBuilder = {
    http("DataToPage")
      .get(s"$baseUrl/sdes/create-request/where-data-to")
      .check(status.is(200))
      .check(RequestUtils.saveCsrfToken)
      .check(regex("Where is the data going").exists)
  }
  def postDataTo: HttpRequestBuilder = {
    http("EnterDataToPage")
      .post(s"$baseUrl/sdes/create-request/where-data-to")
      .formParam("field", "Test data too")
      .formParam("csrfToken", "${csrfToken}")
      .check(status.is(303))
...
//  }
  def postPhysicalDERAdditonalInformation: HttpRequestBuilder = {
    http("PhysicalDERAdditionalInformation")
      .post(s"$baseUrl/sdes/create-request/additional-information")
      .formParam("field", "helllo")
      .formParam("csrfToken", "${csrfToken}")
      .check(status.is(303))
      .check(header("Location").is(s"/sdes/create-request/check-answers":String))
  }
  def getPhysicalCYAPage: HttpRequestBuilder = {
    http("PhysicalCYA")
      .get(s"$baseUrl/sdes/create-request/check-answers")
      .check(status.is(200))
      .check(RequestUtils.saveCsrfToken)
      .check(regex("Check your answers").exists)
  }
    def postPhysicalDERSubmit: HttpRequestBuilder = {
    http("SubmitDER ")
      .post(s"$baseUrl/sdes/create-request/check-answers")
      .formParam("csrfToken", "${csrfToken}")
      .check(status.is(303))
  }
